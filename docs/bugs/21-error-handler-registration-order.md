...
`middleware.setupErrorHandler` is exported separately and called from `main.coffee` after `routing.setup`, where Express will select it. It honours `err.status`/`err.statusCode`: making the handler live would otherwise have turned every body-parser 400 into a 500, including the webhook's.
