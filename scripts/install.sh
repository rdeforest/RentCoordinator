...
create_config() {
    ...
    run_as_root tee "$PREFIX/.env" > /dev/null << EOF
...
create_management_script() {
    ...
    run_as_root cp "$SCRIPT_DIR/lib/rentcoordinator.template" "$PREFIX/bin/rentcoordinator"

    run_as_root sed -i "s|PREFIX_PLACEHOLDER|$PREFIX|g" "$PREFIX/bin/rentcoordinator"
    run_as_root sed -i "s|LOG_DIR_PLACEHOLDER|$LOG_DIR|g" "$PREFIX/bin/rentcoordinator"
    run_as_root sed -i "s|USER_PLACEHOLDER|$APP_USER|g" "$PREFIX/bin/rentcoordinator"

    run_as_root chmod +x "$PREFIX/bin/rentcoordinator"

    if is_root; then
        ln -sf "$PREFIX/bin/rentcoordinator" /usr/local/bin/rentcoordinator
        print_info "Created symlink in /usr/local/bin"
    else
        run_as_root ln -sf "$PREFIX/bin/rentcoordinator" /usr/local/bin/rentcoordinator
        print_info "Created symlink in /usr/local/bin"
    fi
